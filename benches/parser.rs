...
use graphql_tools::parser::{minify_document, minify_query, parse_query, query::Document};
use std::fs::File;
...
fn bench_parser(c: &mut Criterion) {
    ...
    let cases = [
        ("minimal", load_file("minimal")),
        ("inline_fragment", load_file("inline_fragment")),
        ("directive_args", load_file("directive_args")),
        ("query_vars", load_file("query_vars")),
        ("kitchen-sink", load_file("kitchen-sink")),
    ];
...
fn bench_minifiers(c: &mut Criterion) {
    ...
    group.bench_function("minify_document", |b| {
        b.iter(|| minify_document(black_box(&parsed)))
    });
